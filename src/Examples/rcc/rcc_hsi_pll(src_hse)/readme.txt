...
硬件资源：
          ...
          4. PLL作为系统时钟时，PLL输入参考时钟选择HSE时钟（16MHz），经过2倍频，PLL输出32MHz时钟。PCLK、HCLK不分频，PCLK=HCLK=SysClk=32MHz，此时需要注意设置FLASH的读访问等待周期，否则程序无法运行。
...
+ MDK-ARM
          1. 打开rcc_hsi_pll(src_hse).uvproj文件
