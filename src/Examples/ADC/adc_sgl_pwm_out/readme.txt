...
硬件资源：
          ...
          2. 时钟HSI
          3. 系统时钟源为PLL，PLL时钟源为HSI时钟6分频：8MHz，PLL8倍频，PCLK、HCLK不分频，PCLK=HCLK=SysClk=64MHz
          4. GTIM1、GTIM2的时钟源为PCLK，1024分频
          5. PB09设置为PUSH-PULL输出
          6. ADC的时钟源为PCLK，128分频

演示说明：
          通用定时器GTIM1定时1s自动触发ADC模块进行转换，ADC通道为AIN1:PA01。
		  通用定时器GTIM2将AIN1的ADC采样值转换成频率可变的PWM方波，占空比50%。
		  ADC采样值为0时，PWM方波频率为1Hz；ADC采样值为4095时，PWM方波频率为5KHz。
          PB09输出电平翻转，LED1将0.5Hz的频率闪烁。
